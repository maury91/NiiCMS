...
foreach ($__plugins['mobile']['act']['before_control'] as $p) include("plugin/$p.php");
...
if (!strcmp($line['nick'],$_GET['nick']))
{
	if (!strcmp($line['authcode'],$_GET['code']))
	{
		...
		foreach ($__plugins['mobile']['act']['on_auth'] as $p) include("plugin/$p.php");
