...
if ($user['logged']) 
	...
	if (isset($_GET['plg'])) 
			foreach ($__plugins['mobile']['login']['plg'] as $p) include("plugin/$p.php");
	else if ((isset($_POST['next']))||(isset($_GET['next']))) {
		$to_next = '<a href="zone_login.html">'.$__return.'</a>';
						...
						if (($terrs > 2)&&(!captcha_control($idc,$cap))) echo $__err_captcha.$to_next;
							...
							if (!strcmp($udata['password'],md5('gmpass'.md5($upass)).':'.md5(md5($upass).'pass'))) {
								...
								if (!$risultato)	
									...
									foreach ($__plugins['mobile']['login']['success'] as $p) include("plugin/$p.php");
		...
		$_lp_url = "zone_lostp.html";
	?>
		<center><h4><?php echo$__title?></h4><form name="login" method="post" action="	zone_login.html"><p><?php echo$__nick?><input type="hidden" name="next"><input class="textbox" type="text" id="nick" name="nick"></p><p><?php echo$__pass?><input class="textbox" type="password" id="pass" name="pass"></p>
		<?php echo$add?><input class="button" type="submit" value="<?php echo$__submit?>"></form><center><p><?php echostr_replace('%url%',$_lp_url,$__l_pass)?></p></center>
		<?php foreach ($__plugins['mobile']['login']['insert_data'] as $p) include("plugin/$p.php"); ?>
	<?php
	}
}
?>