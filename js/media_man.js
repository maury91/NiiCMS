		...
		var
			...
			readFile = function(file,elem) {			
				if (file) {
					...
					xhr.onreadystatechange = function(e) {
						if ( 4 == this.readyState ) {
							...
							if (response.success) {
								...
								if (cla == "load")  
									add = "<img src='" + mediam_dir + "/" + newn + "' class='media_man_imm' />";
								else add = "";
					...
					url = 'zone_media_man.html?act=upl&uid='+params.uid+'&d='+mediam_dir+'&myfile='+file.name;
			...
			media_delete = function() {
				...
				if (confirm(l__del)) {
					...
					$.ajax({
						url: "zone_media_man.html?act=del&uid="+media_params.uid + elabq,
						success: function () {
							media_dir(main.get(0),mediam_dir);
						}
					})
